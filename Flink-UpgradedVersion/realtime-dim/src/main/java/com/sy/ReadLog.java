...
public class ReadLog {
    ...
    public static void main(String[] args) {
        ...
        // TODO: 发送数据到kafka
        // 主流
        process.sinkTo(Flink2Kafka.getSinkKafka("log_common")).uid("F2K_common").name("F2K_common");
        // 测流
        process.getSideOutput(actionsTag).sinkTo(Flink2Kafka.getSinkKafka("log_actions")).uid("F2K_actions").name("F2K_actions");
        process.getSideOutput(displaysTag).sinkTo(Flink2Kafka.getSinkKafka("log_displays")).uid("F2K_displays").name("F2K_displays");
        process.getSideOutput(pageTag).sinkTo(Flink2Kafka.getSinkKafka("log_page")).uid("F2K_page").name("F2K_page");
        process.getSideOutput(startTag).sinkTo(Flink2Kafka.getSinkKafka("log_start")).uid("F2K_start").name("F2K_start");
        process.getSideOutput(errTag).sinkTo(Flink2Kafka.getSinkKafka("log_err")).uid("F2K_err").name("F2K_err");
